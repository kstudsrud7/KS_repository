...
// Type: ArcticIT.TribalEnrollment.EntityPlugins.ContactIncarcerationPlugin
...
{
  [CRMMetadata(EntityName = "ait_contactincarceration", CrmEntityType = "ArcticIT.EarlyBound.Entities.ait_contactincarceration")]
  public class ContactIncarcerationPlugin : EntityBase
  {
    public ContactIncarcerationPlugin(
    ...
    public static bool UpdateContact(
      ...
      ait_contactincarceration contactIncarceration,
      Global.ActionType actionType)
    {
      bool flag = false;
      if (contactIncarceration == null)
        throw new ArgumentNullException("ContactIncarceration cannot be empty.", nameof (contactIncarceration));
      if (contactIncarceration.ait_contact == null)
        return false;
      Contact entity = (Contact) EntityLibrary.GetEntity(crmService, contactIncarceration.ait_contact, false);
      if (entity == null)
        return false;
      ContactPlugin.UpdateIncarcerated(crmService, entity);
      return flag;
    }

    protected override Entity GetBaseImageForUpdate()
    {
      return (Entity) this.GetInputEntity<ait_contactincarceration>(this.ActionType, Global.Stage.PRE);
    }

    protected override Entity GetPreImageForUpdate()
    {
      return (Entity) this.GetEntityFromPreImage<ait_contactincarceration>();
    }

    protected override string GenerateFormattedName(Entity entityInput, Entity preImage)
    {
      string empty = string.Empty;
      ait_contactincarceration contactincarceration1 = (ait_contactincarceration) entityInput;
      ait_contactincarceration contactincarceration2 = (ait_contactincarceration) preImage;
      return FormatLibrary.FormatName(FormatLibrary.GetEntityValue((IOrganizationService) this.CrmService, contactincarceration1.ait_contact, contactincarceration2.ait_contact), FormatLibrary.GetEntityValue((IOrganizationService) this.CrmService, contactincarceration1.ait_prison, contactincarceration2.ait_prison));
    }

    public void UpdateRelatedRecords()
    {
      ContactIncarcerationPlugin.UpdateContact((IOrganizationService) this.CrmService, this.GetInputEntity<ait_contactincarceration>(this.ActionType), this.ActionType);
    }

    private void Validate()
    {
      if (this.GetInputEntity<ait_contactincarceration>().ait_contact == null)
