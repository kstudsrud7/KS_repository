...
namespace ArcticIT.CrmFramework.Libraries
{
  public class FrameworkService : IOrganizationService
  {
    ...
    public FrameworkService Impersonate(string userName)
    {
      ...
      Guid guid = organizationServiceContext.CreateQuery<SystemUser>().Where<SystemUser>((Expression<Func<SystemUser, bool>>) (user => user.FirstName + " " + user.LastName == userName)).Select<SystemUser, Guid>((Expression<Func<SystemUser, Guid>>) (user => user.SystemUserId.Value)).FirstOrDefault<Guid>();
