...
// Type: ManipulationLibrary.RegEx.Replace
...
namespace ManipulationLibrary.RegEx
{
  public sealed class Replace : CodeActivity
  {
    ...
    protected override void Execute(CodeActivityContext executionContext)
    {
      ...
      string replacement = this.Replacement.Get<string>((ActivityContext) executionContext);
      string input = this.Text.Get<string>((ActivityContext) executionContext);
      string str = input;
      try
      {
        str = new Regex(pattern).Replace(input, replacement);
      }
      catch (ArgumentException ex)
      {
        flag = true;
      }
      this.InvalidRegularExpression.Set((ActivityContext) executionContext, flag);
