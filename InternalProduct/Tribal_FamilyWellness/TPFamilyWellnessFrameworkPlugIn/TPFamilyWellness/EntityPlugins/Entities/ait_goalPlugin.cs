...
// Type: TPFamilyWellness.EntityPlugins.Entities.ait_goalPlugin
...
{
  [CRMMetadata(EntityName = "ait_goal", CrmEntityType = "TPFamilyWellness.EarlyBound.ait_goal")]
  public class ait_goalPlugin : EntityBase
  {
    public ait_goalPlugin(
    ...
    {
      this.OnPluginPostUpdating += new EntityBase.PluginOnPostUpdateHandler(this.ait_goalPlugin_OnPluginPostUpdating);
      this.OnPluginPostCreating += new EntityBase.PluginOnPostCreateHandler(this.ait_goalPlugin_OnPluginPostCreating);
      this.OnPluginPreDeleting += new EntityBase.PluginOnPreDeleteHandler(this.Ait_goalPlugin_OnPluginPreDeleting);
    }

    private void Ait_goalPlugin_OnPluginPreDeleting(
      object sender,
      EntityBase.PluginOnPreDeleteHandlerEventArgs e)
    {
    }

    private void ait_goalPlugin_OnPluginPostCreating(
      object sender,
    ...
    private void ait_goalPlugin_OnPluginPostUpdating(
      object sender,
    ...
    {
      this.CreatePlanActivity(this.GetInputEntity<ait_goal>());
    }

    private void SyncPlanActivityOnPostUpdate()
    {
      ait_goal entityFromPostImage = this.GetEntityFromPostImage<ait_goal>();
      ait_goal inputEntity = this.GetInputEntity<ait_goal>();
      using (CrmServiceContext crmServiceContext = new CrmServiceContext(this.CrmService.OrganizationService))
      {
        ParameterExpression parameterExpression1;
        ParameterExpression parameterExpression2;
        ...
        if (aitPlanactivity1 != null)
        {
          ...
          aitPlanactivity2.ait_EndDate = entityFromPostImage.ait_TargetDate;
          ...
          Ait_goalState? statecode = inputEntity.statecode;
          if (statecode.HasValue)
          {
            statecode = inputEntity.statecode;
            Ait_goalState aitGoalState = Ait_goalState.Inactive;
            if (statecode.GetValueOrDefault() == aitGoalState & statecode.HasValue)
            {
    ...
    private void CreatePlanActivity(ait_goal goal)
    {
      this.CrmService.Create((Entity) new ait_planactivity()
      {
        ait_CasePlan = goal.ait_CasePlan,
        ait_FamilyGroup = goal.ait_FamilyGroup,
        ait_Individual = goal.ait_Individual,
        ait_Outcome = goal.ait_Outcome,
        ait_CompletedDate = goal.ait_ActualCompletion,
        ait_StartDate = goal.ait_StartDate,
        ait_EndDate = goal.ait_TargetDate,
        ait_ActivityType = new OptionSetValue(Ait_planactivity_ait_activitytype.Goal.GetHashCode()),
        ait_name = goal.ait_name,
        ait_StatusLabel = MetadataLibrary.GetStatusCodeName((IOrganizationService) this.CrmService, ((IExecutionContext) this.CrmService.PluginContext).PrimaryEntityName, goal.statuscode.Value),
        OwnerId = goal.OwnerId,
        ait_Goal = new EntityReference(((IExecutionContext) this.CrmService.PluginContext).PrimaryEntityName, ((IExecutionContext) this.CrmService.PluginContext).PrimaryEntityId)
      });
    }
  }
}
